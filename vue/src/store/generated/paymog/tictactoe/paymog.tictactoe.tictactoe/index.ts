...
export default {
	...
	getters: {
			...
			return state.OpenGames[JSON.stringify(params)] ?? {}
				...
				getAllGames: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.AllGames[JSON.stringify(params)] ?? {}
		},
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: paymog.tictactoe.tictactoe initialized!')
		...
		async QueryOpenGames({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryOpenGames(query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryOpenGames({...query, 'pagination.key':(<any> value).pagination.next_key})).data
		...
		async QueryAllGames({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryAllGames(query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryAllGames({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'AllGames', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryAllGames', payload: { options: { all }, params: {...key},query }})
				return getters['getAllGames']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryAllGames API Node Unavailable. Could not perform query: ' + e.message)
	...
	gas: "200000" }, memo})
				...
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgMakeMove:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgMakeMove:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		async sendMsgStartGame({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgStartGame(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
		...
		async MsgMakeMove({ rootGetters }, { value }) {
			try {
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgMakeMove:Create Could not create message: ' + e.message)
				}
			}
		},
